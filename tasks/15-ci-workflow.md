...
## Acceptance criteria
- [x] `test -f .github/workflows/ci.yml`.
- [x] `grep -q 'cargo test --workspace' .github/workflows/ci.yml`.
- [x] `grep -q 'windows-latest' .github/workflows/ci.yml && grep -q 'windows-11-arm' .github/workflows/ci.yml`.
- [x] `grep -q 'actions/checkout' .github/workflows/ci.yml`.
- [x] `cargo test --workspace --locked` (the same command CI will run) exits 0 locally.
