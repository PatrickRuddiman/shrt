...
- [x] (Removed) Cross-repo `update-scoop` job. Bucket-side autoupdate is the chosen mechanism — `PatrickRuddiman/PersonalScoopBucket/bucket/shrt.json` declares `checkver` + `autoupdate` blocks pointing at this repo's GitHub Releases and `sha256sums.txt`, and the bucket repo runs `scoop checkver -u shrt` on its own schedule.
