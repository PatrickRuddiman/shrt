...
- [x] Add `#[cfg(test)] mod tests` in both files. `substitute.rs` tests must cover every placeholder form, missing `{1}` exits 64, missing `{ENV:UNSET}` exits 78, default fallback works, `{{`/`}}` literals, `{ENV:}` rejected, whitespace inside `{...}` rejected, `{@}` quoting on args containing spaces and quotes. `argv.rs` tests must cover: `a b c` → 3 args; `"a b c"` → 1 arg; `\\\"x` → `"x`; `\\\\` → `\\`; trailing unterminated `"` (lenient); empty input.
- [x] Add `mod substitute;` and `mod argv;` to `crates/shrt-runner/src/main.rs`.

## Acceptance criteria
- [x] `cargo build -p shrt-runner` exits 0.
- [x] `cargo test -p shrt-runner substitute::tests` passes.
- [x] `cargo test -p shrt-runner argv::tests` passes.
- [x] `test -f crates/shrt-runner/src/substitute.rs && test -f crates/shrt-runner/src/argv.rs`.
- [x] `grep -q 'pub fn substitute' crates/shrt-runner/src/substitute.rs && grep -q 'pub fn tokenize' crates/shrt-runner/src/argv.rs`.
